...
class ReturnProcessingWorkflow:
    """Manages the return process for orders, including authorization, item receipt, and refund issuance."""
    ...
    async def run(self, arg: WorkflowInput) -> str:
        ...
        try:
            ...
            compensations.append((cancel_return_authorization, CancelReturnAuthorizationInput(return_id=authorisation_result)))
            self._current_step = 'D1'
            should_authorisation_result_return_id = True  # TODO: set from a real condition: authorisation_result.return_id
            if should_authorisation_result_return_id:
                self._decisions_taken.append({'branch': 'D1', 'predicate': 'authorisation_result.return_id', 'taken': True})
                ...
                compensations.append((revoke_refund, RevokeRefundInput(refund_id=refund_result)))
            else:
                self._decisions_taken.append({'branch': 'D1', 'predicate': 'authorisation_result.return_id', 'taken': False})
                self._current_step = 'R1'
