...
| `worker.py` | Worker registering the workflow + activities on `order-fulfilment-queue` |
...
The worker polls the `order-fulfilment-queue` task queue; the starter submits a single
...
- Send the workflow's signal(s) (OrderFulfilRequestSignal) to release any wait gates.
