...
class OrderFulfilmentWorkflow:
    ...
    def __init__(self) -> None:
        ...
        self._order_fulfil_request_signal_received: bool = False
    ...
    async def run(self, arg: WorkflowInput) -> str:
        ...
        try:
            ...
            workflow.logger.info("Waiting for signal: OrderFulfilRequestSignal")
            # Bounded wait: raises TimeoutError after DispatchTimeout, which
            # fires the saga compensations below instead of blocking forever.
            await workflow.wait_condition(lambda: self._order_fulfil_request_signal_received, timeout=DISPATCH_TIMEOUT)
            ...
            payment_id = await workflow.execute_activity(
                capture_payment,
                CapturePaymentInput(authorization_id=arg.authorization_id, shipment_id=shipment_id),
                start_to_close_timeout=timedelta(seconds=60),
    ...
    @workflow.signal
    def order_fulfil_request_signal(self, order_id: str) -> None:
        """Handle the 'OrderFulfilRequestSignal' signal."""
        self._order_fulfil_request_signal_received = True

    @workflow.query
    def get_fulfilment_status(self) -> str:
        """Queries the current fulfilment status of an order."""
        return self._status
