...
class ReturnProcessingWorkflow:
    ...
    async def run(self, arg: WorkflowInput) -> str:
        ...
        try:
            ...
            authorise_return_result = await workflow.execute_activity(
                authorise_return,
                AuthoriseReturnInput(order_id=arg.order_id, shipment_id=arg.shipment_id, reason_code=arg.reason_code),
                start_to_close_timeout=timedelta(seconds=10),
            )
            compensations.append((revoke_return_authorisation, RevokeReturnAuthorisationInput()))
            self._current_step = 'D1'
            should_authorise_return_result_return_id_is_not_null = True  # TODO: set from a real condition: authorise_return_result.return_id is not null
            if should_authorise_return_result_return_id_is_not_null:
                self._decisions_taken.append({'branch': 'D1', 'predicate': 'authorise_return_result.return_id is not null', 'taken': True})
                self._current_step = 'A2'
                workflow.logger.info("Running step: ReceiveReturnedItem")
                receive_item_result = await workflow.execute_activity(
                    receive_returned_item,
                    ReceiveReturnedItemInput(shipment_id=arg.shipment_id, return_id=authorise_return_result),
                ...
                issue_refund_result = await workflow.execute_activity(
                    issue_refund,
                    IssueRefundInput(order_id=arg.order_id, return_id=authorise_return_result),
                    start_to_close_timeout=timedelta(seconds=30),
                    retry_policy=RetryPolicy(initial_interval=timedelta(seconds=1), backoff_coefficient=2, maximum_attempts=5),
                )
                compensations.append((cancel_refund, CancelRefundInput()))
            else:
                self._decisions_taken.append({'branch': 'D1', 'predicate': 'authorise_return_result.return_id is not null', 'taken': False})
                self._current_step = 'E1'
                workflow.logger.info("Running step: RevokeReturnAuthorisation")
                e1_result = await workflow.execute_activity(
                    revoke_return_authorisation,
                    RevokeReturnAuthorisationInput(return_id=authorise_return_result),
                    start_to_close_timeout=timedelta(seconds=60),
                )
        except Exception:
    ...
    def get_return_status(self) -> str:
        """Returns the current return status"""
        return self._status
