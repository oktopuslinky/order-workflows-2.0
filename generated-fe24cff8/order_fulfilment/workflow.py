...
class OrderFulfilmentWorkflow:
    ...
    async def run(self, arg: WorkflowInput) -> str:
        ...
        try:
            ...
            should_dispatch_status_accepted = dispatch_status == 'accepted'  # branch condition: dispatch_status == 'accepted'
            if should_dispatch_status_accepted:
                self._decisions_taken.append({'branch': 'S4', 'predicate': "dispatch_status == 'accepted'", 'taken': True})
                ...
                self._decisions_taken.append({'branch': 'S4', 'predicate': "dispatch_status == 'accepted'", 'taken': False})
            ...
            workflow.logger.info("Running step: WaitForCarrierPickup")
            pickup_confirmation = await workflow.execute_activity(
                wait_for_carrier_pickup,
                WaitForCarrierPickupInput(),
