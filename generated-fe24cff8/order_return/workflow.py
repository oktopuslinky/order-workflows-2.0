...
class ReturnProcessingWorkflow:
    ...
    async def run(self, arg: WorkflowInput) -> str:
        ...
        try:
            ...
            should_authorisation_result_return_id_null = True  # TODO: set from a real condition: authorisation_result.return_id != null
            if should_authorisation_result_return_id_null:
                self._decisions_taken.append({'branch': 'D1', 'predicate': 'authorisation_result.return_id != null', 'taken': True})
                ...
                self._decisions_taken.append({'branch': 'D1', 'predicate': 'authorisation_result.return_id != null', 'taken': False})
                self._current_step = 'E1'
                workflow.logger.info("Raising: ReturnNotEligible")
                raise ApplicationError('Raise if return not eligible', type='ReturnNotEligible', non_retryable=True)
        except Exception:
