...
async def authorise_return(arg: AuthoriseReturnInput) -> str:
    """Authorises the return via Returns Service API."""
...
async def receive_returned_item(arg: ReceiveReturnedItemInput) -> str:
    """Notifies Warehouse Service of item return."""
...
async def issue_refund(arg: IssueRefundInput) -> str:
    """Processes refund via Payment Gateway API."""
...
async def cancel_refund(arg: CancelRefundInput) -> str:
    """Cancels a refund if issuance failed or needs reversal."""
...
@activity.defn(name="ReverseAuthorisation")
async def reverse_authorisation(arg: ReverseAuthorisationInput) -> str:
    """Reverses the return authorisation if the process fails afterwards."""
    activity.logger.info("Running reverse_authorisation", extra={"input": arg})
    # TODO: implement reverse_authorisation. Returns a placeholder so the bundle runs as-is.
    return ""
