...
class OrderSettlementWorkflow:
    ...
    async def run(self, arg: WorkflowInput) -> str:
        ...
        try:
            ...
            should_is_settleable = bool(is_settleable)  # branch condition: is_settleable
            if should_is_settleable:
                ...
                payment_id = await workflow.execute_activity(
                    ...
                    retry_policy=RetryPolicy(initial_interval=timedelta(seconds=5), backoff_coefficient=1, maximum_attempts=2, non_retryable_error_types=['PAYMENT_DECLINED']),
                )
                ...
                workflow.logger.info("Waiting for signal: shipping.confirmed")
                ...
                settlement_id = await workflow.execute_activity(
                    finalise_settlement,
                    FinaliseSettlementInput(order_id=arg.order_id, payment_id=payment_id),
                    start_to_close_timeout=timedelta(seconds=60),
                )
            else:
                self._decisions_taken.append({'branch': 'S2', 'predicate': 'is_settleable', 'taken': False})
            self._current_step = 'S12'
            should_payment_successful = True  # TODO: set from a real condition: payment_successful
            if should_payment_successful:
                self._decisions_taken.append({'branch': 'S12', 'predicate': 'payment_successful', 'taken': True})
                ...
                self._decisions_taken.append({'branch': 'S12', 'predicate': 'payment_successful', 'taken': False})
        except Exception:
