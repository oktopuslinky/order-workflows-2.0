...
class ReturnProcessingWorkflow:
    ...
    async def run(self, arg: WorkflowInput) -> str:
        ...
        try:
            ...
            should_authorise_return_result_return_id_null = True  # TODO: set from a real condition: authorise_return_result.return_id != null
            if should_authorise_return_result_return_id_null:
                self._decisions_taken.append({'branch': 'D1', 'predicate': 'authorise_return_result.return_id != null', 'taken': True})
                ...
                receive_item_result = await workflow.execute_activity(
                    receive_returned_item,
                    ReceiveReturnedItemInput(shipment_id=arg.shipment_id),
                ...
                issue_refund_result = await workflow.execute_activity(
                    issue_refund,
                    IssueRefundInput(order_id=arg.order_id),
                ...
                self._decisions_taken.append({'branch': 'D1', 'predicate': 'authorise_return_result.return_id != null', 'taken': False})
                self._current_step = 'E1'
                workflow.logger.info("Raising: ReturnNotEligible")
                raise ApplicationError('Raise if return is not eligible.', type='ReturnNotEligible', non_retryable=True)
        except Exception:
    ...
    def get_return_status(self) -> str:
        """Query to get the current return status."""
        return self._status
