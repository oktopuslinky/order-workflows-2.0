...
## Decisions
- [d1] Dispatch outcome — after: a3; yes: a4; no: e1
...
- [ev2] carrier.picked_up — kind: signal_wait; emitted by: a4
...
- [x] uses output `order_id` of `order-placement` as input `order_id` — Order Fulfilment requires the order_id produced by Order Placement to initiate fulfillment
- [x] provides output `shipment_id` to `order-return` input `shipment_id` — Order Return requires the shipment_id from Order Fulfilment to complete the return process
