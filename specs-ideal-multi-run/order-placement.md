...
- start states: Cart submitted for checkout, Cart in open state
...
- [x] provides output `order_id` to `order-fulfilment` input `order_id` — Order Fulfilment requires the order_id produced by Order Placement to initiate fulfillment
