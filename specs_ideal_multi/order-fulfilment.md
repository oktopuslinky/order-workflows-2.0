...
## Purpose
The Order Fulfilment workflow ships a placed order and captures its payment. It picks, packs, and dispatches items, rolling back if dispatch fails.
...
- triggers: order.fulfil request received by Fulfilment Service
- start states: placed
...
## Decisions
- [d1] Dispatch successful? — after: a3; yes: a4; no: e1
...
## Events
- [v1] [ev1] carrier.picked_up — emitted by: a5

## State Transitions
- placed -> fulfilment_in_progress
- fulfilment_in_progress -> shipped (trigger: a4)
- fulfilment_in_progress -> cancelled (trigger: e1)
- fulfilment_in_progress -> cancelled (trigger: e2)
...
- [ ] uses output `order_id` of `order-placement` as input `order_id` — Order Fulfilment consumes the order_id produced by Order Placement
- [ ] provides output `shipment_id` to `order-return` input `shipment_id` — Order Return consumes the shipment_id produced by Order Fulfilment
